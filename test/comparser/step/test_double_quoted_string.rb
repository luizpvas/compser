...
class Comparser::Step::TestDoubleQuotedString < Minitest::Test
  include Comparser

  def test_single_word
    parser = succeed.and_then(:double_quoted_string)

    parser.call(Comparser::State.new('"hello"')).tap do |state|
      assert state.good?
  ...
  def test_line_break
    ...
    parser.call(Comparser::State.new(source)).tap do |state|
      assert state.good?
